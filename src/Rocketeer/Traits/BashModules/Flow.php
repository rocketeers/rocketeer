...
trait Flow
{
    ...
    public function share($file)
    {
        ...
        if (!$this->fileExists($sharedFile)) {
            // If the file exists on the prvious release, use it. (Example: files generated by the previous release)
            $previousFile = $this->releasesManager->getPathToRelease($this->releasesManager->getPreviousRelease().'/'.$file);
            if ($this->fileExists($previousFile)) {
                $this->move($previousFile, $sharedFile);
            }
            elseif ($this->fileExists($currentFile)) {
                $this->move($currentFile, $sharedFile);
            }
            else {
                $this->explainer->line('Sharing file failed. File not found: '.$currentFile);
                return false;
            }
        }
